...
function analyzeJs(rootDir,file,source) {
  ...
  function secondPass(node,parent=null,current=null) {
    ...
    // worker_threads.Worker constructs a thread that executes code: a real
    // execution capability, modeled like child_process execution.
    if(node.type==="NewExpression"){
      let wmod=null, wname=null;
      if(node.callee.type==="Identifier"){ const b=importedFns.get(resolveAlias(node.callee.name)); if(b){wmod=b.module;wname=b.name;} }
