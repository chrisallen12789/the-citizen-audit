...
**OPEN — RECOMMENDED layered OS-native Linux confinement composition documented; pending independent review and explicit project-owner approval.**
...
2. The supervisor selects an exact platform profile, discovers capabilities, and refuses the attempt if any mandatory control or evidence hook is unavailable.
3. The supervisor assigns an exact attempt identity, validates and immutably stages exact artifacts, constructs bounded transports and storage, and creates the durable process and resource ownership domain.
4. The ownership layer and kernel establish filesystem, process, IPC, network, identity, handle, syscall, access-control, diagnostic, and resource policies before untrusted instructions execute.
...
8. Every outcome triggers complete tree termination, cleanup, residual scans, and audit finalization. Cleanup detection does not excuse a prevention failure; it determines whether success must remain blocked and remediation is required.
...
Before launch, the supervisor must identify the requested profile and verify all required platform facts, ownership primitives, resource controllers, namespace or equivalent boundaries, identity reduction, non-escalation, descriptor policy, artifact immutability, network denial, local IPC isolation, syscall policy, mandatory-access-control policy, dump policy, transport bounds, audit availability, cleanup authority, and evidence hooks.
...
1. The exact approved platform and confinement profile was identified and all mandatory capabilities were verified before launch.
...
4. Mandatory filesystem, process-tree, resource, identity, non-escalation, environment, inherited-handle, network, local IPC, syscall, mandatory-access-control, dump, and transport controls were active before untrusted execution and remained active through termination.
...
7. The complete process tree was terminated and all required workspaces, transports, IPC names, policy objects, dumps, diagnostics, and other attempt state were removed or generation-safely reconciled.
...
If and only if independent review, owner approval, dependency reconciliation, and required evidence support the recommendation, a future approval record could state:

> P42-D003 is resolved at the architecture-class level by requiring a layered, OS-native Linux confinement profile under a dedicated external supervisor or launcher, with durable generation-safe process-tree ownership, per-attempt resource enforcement, isolated filesystem, process, IPC, and network authority, least-privilege launch with monotonic non-escalation, explicit inherited-handle allowlisting, immutable exact-byte staging, syscall and mandatory-access-control policy, bounded supervisor-owned transports, fail-closed cleanup, and mandatory assurance evidence before success acceptance. Exact mechanisms and platform profiles remain governed by their separately approved decisions and independently reproduced evidence.
