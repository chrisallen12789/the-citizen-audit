...
**Open boundary condition.** The inspected attempt record binds `validatorSetHash`, while the worker loader tracks an internal module generation. The inspected durable validation result stores phase results and hashes, not an explicit validator-entry generation. The required binding between an invalidated exact entry generation and an attempt's successful transport/outcome has not been established. That is [VAL-RESULT-001](phase41-invariant-catalog.md#val-result-001-attempt-success-is-bound-to-a-valid-validator-entry-generation), OPEN.
...
| CA-05 | An invalidated exact validator-entry generation cannot yield a successful attempt result or committed attempt. | Without it, a validation decision can outlive the entry generation it purportedly evaluated. | A durable attempt-generation binding, invalidation checks before transport and commit, and race-focused regressions. | The task’s controlling status identifies the invariant; source inspection shows `validatorSetHash` and internal module generations but no demonstrated attempt-generation binding. | **OPEN** | Design, implement, and independently reproduce a fail-closed binding at result transport and attempt outcome. |
