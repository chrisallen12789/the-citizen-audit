...
The intended boundary starts when the execution orchestrator prepares an immutable validation request and asks a supervisor to launch a confined validator process. It ends when the supervisor has validated a bounded result, terminated and accounted for the process tree, and completed cleanup. The supervisor, host operating system, and selected enforcement mechanisms are outside the validator's authority.
...
3. On a parseable result and normal exit, the supervisor verifies exit semantics, provenance binding, output bounds, and cleanup preconditions before accepting success.
...
Assumptions are that a supported host can enforce the selected controls, the TCB is configured correctly, the supervisor can identify and terminate the relevant process tree, and independent reviewers can reproduce evidence. **OPEN:** authoritative production operating system, exact mechanism set, and numerical budgets are not yet chosen.
